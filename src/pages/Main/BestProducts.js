...
export default function BestProducts() {
  ...
  useEffect(() => {
    fetch('http://10.58.3.54:8000/products?ordering=-best_ranking')
      .then(res => res.json())
  ...
  // console.log('베수투', bestProduct);
  return (
    <div>
      <PopularAndAllText>
        <PopularText>성호 추천 인기 프렙 👍</PopularText>
        <p onClick={goToList}>전체보기</p>
      </PopularAndAllText>
      <ProductBox>
        {bestProduct.results &&
          bestProduct.results.slice(0, 4).map((ele, index) => {
            return (
              <Product
                onClick={() => {
